...
def execute_clean_data(data_set_names):
    for data_set_name in data_set_names:
        ...
        if not Path(base_path).exists():
            out = subprocess.run(["python", "clean_data.py", "--data_set_name", f"{data_set_name}"], capture_output=True, text=True)
            print(out.stdout)
            print(out.stderr)
            ...
            if not Path(base_path).exists():
                out = subprocess.run(
                    ["python", "prune_data.py", "--data_set_name", f"{data_set_name}", "--prune_technique",
                     f"{prune_technique}"], capture_output=True, text=True)
            print(out.stdout)
            print(out.stderr)
                ...
                def run_script(reproducibility_seed):
                    out = subprocess.run(
                        ["python", "generate_splits.py", "--data_set_name", f"{data_set_name}", "--prune_technique",
                         ...
                         f"{num_folds}", "--reproducibility_seed", f"{reproducibility_seed}"], capture_output=True, text=True)
            print(out.stdout)
            print(out.stderr)
                                    ...
                                    if not Path(base_path).exists():
                                        out = subprocess.run(
                                            ["python", "fit_recommender.py", "--data_set_name",
                                             ...
                                             f"{reproducibility_seed}"], capture_output=True, text=True)
                                        print(out.stdout)
                                        print(out.stderr)
                                    ...
                                    if not Path(base_path).exists():
                                        out = subprocess.run(
                                            ["python", "make_predictions.py", "--data_set_name",
                                             ...
                                             f"{num_batches}", "--run_batch", f"{run_batch}"], capture_output=True, text=True)
                                        print(out.stdout)
                                        print(out.stderr)
                                ...
                                if not Path(base_path).exists():
                                    ...
                                    out = subprocess.run(
                                        ["python", "evaluate_predictions.py", "--data_set_name",
                                         ...
                                         "--topn_scores"] + [f"{r}" for r in topn_scores], capture_output=True, text=True)
                                    print(out.stdout)
                                    print(out.stderr)
